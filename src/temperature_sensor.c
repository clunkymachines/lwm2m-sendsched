...
static const struct lwm2m_obj_path temperature_path =
	LWM2M_OBJ(TEMPERATURE_OBJECT_ID, TEMPERATURE_INSTANCE_ID, TEMPERATURE_RES_VALUE);
...
int temperature_sensor_init(void)
{
	...
	ret = lwm2m_create_object_inst(&LWM2M_OBJ(TEMPERATURE_OBJECT_ID, TEMPERATURE_INSTANCE_ID));
	if (ret < 0 && ret != -EEXIST) {
		LOG_ERR("Failed to create temperature object instance (%d)", ret);
		return ret;
	}

	(void)lwm2m_set_string(&LWM2M_OBJ(TEMPERATURE_OBJECT_ID, TEMPERATURE_INSTANCE_ID,
