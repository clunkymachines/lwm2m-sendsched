...
struct send_sched_rule_entry rule_entries[SEND_SCHED_RULES_MAX_INSTANCES];

static int send_sched_collect_paths(struct lwm2m_obj_path *paths,
				    size_t max_paths);
...
bool send_sched_rule_parse_int(const char *rule, const char *attr,
			       int32_t *out_value)
{
	size_t attr_len;
	char *end = NULL;
