...
LOG_MODULE_DECLARE(send_scheduler, CONFIG_SEND_SCHED_LOG_LEVEL);
...
static struct lwm2m_engine_res_inst send_sched_ctrl_res_inst
	[SEND_SCHED_CTRL_RES_INST_COUNT];
...
static struct lwm2m_engine_res
	send_sched_rules_res[SEND_SCHED_RULES_MAX_INSTANCES]
			    [SEND_SCHED_RULES_RES_COUNT];
static struct lwm2m_engine_res_inst
	send_sched_rules_res_inst[SEND_SCHED_RULES_MAX_INSTANCES]
				 [SEND_SCHED_RULES_RES_INST_COUNT];
static struct lwm2m_engine_obj_inst
	send_sched_rules_inst[SEND_SCHED_RULES_MAX_INSTANCES];
...
static int send_sched_flush_cb(uint16_t obj_inst_id, uint8_t *args,
			       uint16_t args_len)
{
...
static struct lwm2m_engine_obj_inst *
send_sched_ctrl_create(uint16_t obj_inst_id)
{
	static bool created;
	int i = 0;
	int j = 0;
	...
	if (created || obj_inst_id != 0U) {
		LOG_WRN("Scheduler control instance %u already exists or not 0",
			obj_inst_id);
		return NULL;
	}
	...
	(void)memset(&send_sched_ctrl_inst, 0, sizeof(send_sched_ctrl_inst));
		init_res_instance(send_sched_ctrl_res_inst,
				  ARRAY_SIZE(send_sched_ctrl_res_inst));
...
send_sched_rules_create(uint16_t obj_inst_id)
{
	...
	if (avail < 0) {
		LOG_WRN("No slot available for sampling rules instance %u",
			obj_inst_id);
		return NULL;
	}
	...
	(void)memset(&send_sched_rules_inst[avail], 0,
		     sizeof(send_sched_rules_inst[avail]));

	k_work_init_delayable(&rule_entries[avail].pmax_work,
			      send_sched_pmax_work_handler);
